...
def main(model, path_to_test):
    ...
    for sample in tqdm(ds):
        ...
        # Sample random index
        idx = torch.randint(0, len(input_ids)-1, size=(1,), dtype = int).item()
    ...
    # accuracy, recall, precision, f1 = map(lambda x:np.mean(x), [accuracies, recalls, precisions, f1s])
